...
local function DrawScreeningPanel(panel, width, height, headerText)
	...
	-- Typewriter effect for lines in Aurebesh
	local cycle = 8.0
...
local function ApplyCharMenuStatic(panel)
    ...
    -- Manually apply margins to match the footers (Scale(8))
    -- Remove FitModelContainer calls to avoid conflicts
    ...
    -- We detect this by checking if it's a ScrollPanel, which suggests it's the inner part of our new layout
...
function PANEL:Init()
	...
	self.factionModel:SetFOV(modelFOV - 35) -- Matches visual style of Description/Attributes
...
function PANEL:Populate()
	...
	if (!self.payload.faction) then
		for _, v in pairs(self.factionButtons) do
			if (v.faction == self.payload.faction) then
				v:DoClick(v)
				break
			end
		end
	...
	-- Last ditch attempt to ensure a faction is selected
if (!lastSelected and #self.factionButtons > 0) then
