...
ITEM.isArcCWWeapon = true   -- required
ITEM.attachments = {}       -- list of default attachments relative to their slots
...
function ITEM:AddAttachment(slot, att)
    local atts = self:GetAttachments()
    atts[slot] = att
    self:SetData("attachments", atts)
end

function ITEM:RemoveAttachment(slot)
    local atts = self:GetAttachments()
    atts[slot] = nil
    self:SetData("attachments", atts)
end
...
function ITEM:OnPostLoadout()
    if (self:GetData("equip")) then
        ...
        if weapon and (IsValid(weapon)) then
            ...
            ix.arccw.InitWeapon(client, weapon, self)
        else
